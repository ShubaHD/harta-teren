...
-- ========== 18. CSV ECHIPARE + PRIORITATE (024) ==========
...
-- ========== 19. PRESSUREMETER TEST (025) ==========
ALTER TABLE drill_points ADD COLUMN IF NOT EXISTS pressuremeter_test TEXT;

-- ========== 20. FIELD SITE FILES – fișă PDF extra (026) ==========
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_field_site_files_one_pdf ON field_site_files(drill_point_id);
...
CREATE POLICY "public_read_field_site_files" ON field_site_files FOR SELECT USING (true);
DROP POLICY IF EXISTS "admin_insert_field_site_files" ON field_site_files;
CREATE POLICY "admin_insert_field_site_files" ON field_site_files FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "admin_update_field_site_files" ON field_site_files;
CREATE POLICY "admin_update_field_site_files" ON field_site_files FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')) WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "admin_delete_field_site_files" ON field_site_files;
CREATE POLICY "admin_delete_field_site_files" ON field_site_files FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('field-site-files', 'field-site-files', true, 83886080, ARRAY['application/pdf'])
ON CONFLICT (id) DO UPDATE SET file_size_limit = EXCLUDED.file_size_limit, allowed_mime_types = EXCLUDED.allowed_mime_types;
DROP POLICY IF EXISTS "field_site_files_storage_select" ON storage.objects;
CREATE POLICY "field_site_files_storage_select" ON storage.objects FOR SELECT USING (bucket_id = 'field-site-files');
DROP POLICY IF EXISTS "field_site_files_storage_insert" ON storage.objects;
CREATE POLICY "field_site_files_storage_insert" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'field-site-files' AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "field_site_files_storage_update" ON storage.objects;
CREATE POLICY "field_site_files_storage_update" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'field-site-files' AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
DROP POLICY IF EXISTS "field_site_files_storage_delete" ON storage.objects;
CREATE POLICY "field_site_files_storage_delete" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'field-site-files' AND EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- ========== 27. ACCES PE PROIECT (027) ==========
...
CREATE POLICY "users_read_own_project_access" ON project_access FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS "admin_all_project_access" ON project_access;
CREATE POLICY "admin_all_project_access" ON project_access FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT
    ...
    OR EXISTS (SELECT 1 FROM project_access WHERE user_id = auth.uid() AND project_id = p_id);
$$;
...
CREATE POLICY "teams_read_projects" ON projects FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'team') AND public.can_access_project(id));
DROP POLICY IF EXISTS "teams_read_points" ON drill_points;
CREATE POLICY "teams_read_points" ON drill_points FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'team') AND public.can_access_project(project_id));
DROP POLICY IF EXISTS "teams_update_points" ON drill_points;
CREATE POLICY "teams_update_points" ON drill_points FOR UPDATE TO authenticated
  USING (
    public.can_access_project(project_id)
    AND (
      status = 'de_facut'
      OR (status IN ('in_lucru', 'finalizat') AND (
